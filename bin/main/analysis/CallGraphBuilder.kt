...
object CallGraphBuilder {
    ...
    fun build(project: Project, phpPsiFiles: List<PsiFile>, defs: DefinitionIndex): CallGraph {
        ...
        for (psiFile in phpPsiFiles) {
            ...
            psiFile.accept(PhpCallVisitor(
                ...
                onNewClassResolved = { resolved ->
                    ...
                    if (resolved is PhpClass) {
                        ...
                        if (ctor != null) {
                            val mid = MethodId(resolved.fqn, ctor.name, ctor.isStaticMethod)
                    ...
                    onNewClassResolved = { resolved ->
                        ...
                        if (resolved is PhpClass) {
                            resolved.constructor?.let { ctor ->
                                addEdge(from, MethodId(resolved.fqn, ctor.name, ctor.isStaticMethod))
                            }
                        }
                    }
                ))
            ...
            for (m in methods) {
                ...
                val from = MethodId(ownerFqn = owner, name = m.name, isStatic = m.isStaticMethod)
    ...
    private fun resolvedToSymbolId(resolved: PsiElement): SymbolId? = when (resolved) {
        is Function -> {
            if (resolved.context is PhpClass) null
            else resolved.fqn?.let { FunctionId(it) }
        }
        is Method -> {
            val owner = resolved.containingClass?.fqn
            if (owner.isNullOrBlank()) null else MethodId(ownerFqn = owner, name = resolved.name, isStatic = resolved.isStaticMethod)
        }
        is PhpClass -> {
            // Check if it's a trait
            try {
                if (resolved.isTrait) {
                    resolved.fqn?.let { TraitId(it) }
                } else {
                    resolved.fqn?.let { ClassId(it) }
                }
            } catch (_: Throwable) {
                // If isTrait doesn't exist, treat as class
                resolved.fqn?.let { ClassId(it) }
            }
        }
        else -> null
